...
const DropArea = ({ onDrop, position, emptyColumn, children }) => {
  ...
  if (emptyColumn) {
    return (
      <div 
        ref={drop}
        className={`empty-column-drop-zone ${isOver ? 'empty-column-drop-zone--active' : ''}`}
        style={{
          minHeight: '80px',
          display: 'flex',
          alignItems: 'center',
          ...
          margin: '8px',
          padding: '16px',
          backgroundColor: isOver ? '#f5f9ff' : '#fafafa',
          transition: 'all 0.2s ease',
          color: isOver ? '#2196F3' : '#9e9e9e',
          fontSize: '14px',
          fontWeight: 500,
        }}
      >
  ...
  return (
    <div 
      ...
      style={{
        height: '10px',
        margin: '2px 0',
        backgroundColor: isOver ? '#e1f5fe' : 'transparent',
        borderRadius: '4px',
...
const Column = ({ column, onDrop, onEdit, onCardAdded, boardId }) => {
  ...
  const isEmpty = !Array.isArray(column.tasks) || column.tasks.length === 0;

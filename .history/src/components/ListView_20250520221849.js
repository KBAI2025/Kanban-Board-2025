...
const ListView = ({ board, onBoardUpdate }) => {
  ...
  // Default sort by priority (Critical at top)
  const [sortConfig, setSortConfig] = useState({ 
    key: 'priority', 
    direction: 'desc' 
  });

  // Ensure priority filter is set to 'all' by default
  useEffect(() => {
      ...
      return tasks
        ...
        .map(task => {
          ...
          return {
            ...
            priorityIndex: getPriorityIndex(task.priority)
          };
        });
    });
  ...
  const allTasks = useMemo(() => {
    ...
    return [...processedTasks].sort((a, b) => {
      ...
      if (a.columnOrderIndex > b.columnOrderIndex) return 1;
      if (a.columnOrderIndex < b.columnOrderIndex) return -1;
      
      // Then apply the configured sort if any
      if (sortConfig.key) {
        let aValue, bValue;
        ...
        switch (sortConfig.key) {
          case 'title':
            ...
            break;
          case 'epicLabel':
            aValue = a.epicLabel?.toLowerCase() || '';
            bValue = b.epicLabel?.toLowerCase() || '';
            break;
          case 'priority':
            aValue = a.priorityIndex;
            bValue = b.priorityIndex;
            break;
          case 'assignee':
      ...
      if (a.priorityIndex < b.priorityIndex) return -1;
      if (a.priorityIndex > b.priorityIndex) return 1;
